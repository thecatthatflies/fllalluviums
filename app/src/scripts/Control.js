...
import { borderRadius, SampleHeight, halfSize } from "../scene/SeaFloor.js";
import { animals } from "../scene/Wildlife.js";
...
export function Update() {
    ...
    // ---- Water surface barrier: prevent going above water ----
    const surfaceLimit = -0.3;
    if (camera.position.y > surfaceLimit) {
        camera.position.y = surfaceLimit;
    }

    // ---- Sea floor collision ----
    const gridX = camera.position.x + halfSize;
    const gridZ = camera.position.z + halfSize;
    const floorHeight = SampleHeight(gridX, gridZ);
    const playerHeightOffset = 2.0;
    if (camera.position.y < floorHeight + playerHeightOffset) {
        camera.position.y = floorHeight + playerHeightOffset;
    }

    // ---- Animal collision (fish / sharks) ----
    const animalPushRadius = 1.0;
    for (let ai = 0; ai < animals.length; ai++) {
        const creature = animals[ai];
        const aPos = creature.mesh.position;
        const r = creature.boundingRadius + animalPushRadius;
        const dx = camera.position.x - aPos.x;
        const dy = camera.position.y - aPos.y;
        const dz = camera.position.z - aPos.z;
        const distSq = dx * dx + dy * dy + dz * dz;
        if (distSq < r * r && distSq > 0.001) {
            const dist = Math.sqrt(distSq);
            const pushDist = r - dist;
            camera.position.x += (dx / dist) * pushDist;
            camera.position.y += (dy / dist) * pushDist;
            camera.position.z += (dz / dist) * pushDist;
        }
    }
